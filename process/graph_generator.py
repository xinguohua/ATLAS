		...
		if len(splitted_line[3]) > 0:
			...
			if len(splitted_line[8]) > 0:
				...
				if not G.has_node(joint_ips1) and not G.has_node(joint_ips2):
					if int(splitted_line[0]) >= StartTime:
						joint_ips = f"\"connection_{joint_ips1}\""
...
def load_user_artifact(file):
	ua_file = open("raw_logs/"+ file + "/user_artifact.txt")
...
def load_hosts_ips(file):
	global hosts_ips
	hosts_ips = []
	ip_file = open("raw_logs/" + file + "/ips.txt")
...
if __name__ == '__main__':
	user_node = ""
	fileList = os.listdir("preprocess_logs")
	for file in os.listdir("preprocess_logs"):
		start = time.time()

		# we always assume that investigation starts at h1
		...
		total_nodes_in_sets = ""
		log_file_name = "./preprocess_logs/" + file
		if os.path.exists(log_file_name):
			log_file = open(log_file_name, "r")
			print("文件大小:", os.path.getsize(log_file_name), "字节")
		...
		# 生成uuid
		# for n in ROOT_G_subgraph.nodes():
		# 	if 'uuid' not in ROOT_G_subgraph.nodes[n]:
		# 		ROOT_G_subgraph.nodes[n]['uuid'] = str(uuid.uuid4())
		# save graph in dot format
		nx.nx_pydot.write_dot(ROOT_G_subgraph, "graph/graph_" + file + ".dot")
